...
| 页面初始化时报 `Cannot read properties of undefined (reading 'map')` | 先看 `/open_api/settings` 返回是否正常。如果是 Worker 直连部署，通常是 worker 变量没有设置成功，请检查 `DOMAINS`、`ADMIN_USER_EMAILS` 等 JSON 格式变量是否正确配置；如果是 Pages 前端部署并且请求打到了错误地址，则继续看下方 Pages 相关排障 |
...
| 登录后访问 `/admin` 提示「无权访问」 | 管理后台采用基于角色的访问控制，没有管理员密码。请确认该账号邮箱在 Worker 变量 `ADMIN_USER_EMAILS` 中（重新登录后自动获得管理员角色），或由现有管理员在「管理后台 → 用户」中给该账号分配 `ADMIN_USER_ROLE` 对应的角色。详见 [管理后台](/zh/guide/feature/admin) |
| 调用 `/admin/*` 接口返回 401 | 请求需要携带 `x-user-access-token` 且令牌角色为管理员；旧的 `x-admin-auth` 请求头与 `/open_api/admin_login` 接口已移除。获取方式见 [管理后台 → 在脚本中调用 Admin API](/zh/guide/feature/admin#在脚本中调用-admin-api) |
