...
| `Cannot read properties of undefined (reading 'map')` during page initialization | First check whether `/open_api/settings` is returning valid data. In a direct Worker deployment, this usually means Worker variables were not configured correctly, so verify JSON-format variables such as `DOMAINS` and `ADMIN_USER_EMAILS`. If this happens in a Pages deployment because requests are going to the wrong backend address, continue with the Pages troubleshooting section below |
...
| Visiting `/admin` after signing in shows "No access" | The Admin Console is role-based; there is no admin password. Make sure the account email is listed in the `ADMIN_USER_EMAILS` Worker variable (the admin role is granted automatically after signing in again), or have an existing admin assign the role matching `ADMIN_USER_ROLE` to the account in **Admin Console → Users**. See [Admin Console](/en/guide/feature/admin) |
| `/admin/*` API calls return 401 | Requests must carry `x-user-access-token` with the admin role; the old `x-admin-auth` header and the `/open_api/admin_login` endpoint have been removed. See [Admin Console → Calling Admin APIs from Scripts](/en/guide/feature/admin#calling-admin-apis-from-scripts) |
